...
if __name__ == "__main__":
    ...
    parser.add_argument("--outf",
        default="./output2/BLUE4/",
        help='where to store the output')
    parser.add_argument("--data",
        default='../data_generation/blenderproc_data_gen/output2/test44/',
        # default='./test',
        help='folder for data images to load, *.png, *.jpeg, *jpg')
    parser.add_argument("--config",
        default="config_inference2/config_pose.yaml",
        help='folder for the inference configs')
    parser.add_argument("--camera",
        default="config_inference2/camera_info.yaml",
    ...
    def create_output_path(input_path, output_base):
        """
        Creates the output path by replicating the input path's directory structure under the output base directory.
        """
        rel_path = os.path.relpath(input_path, opt.data)
    ...
    if opt.data is not None:
        videopath = opt.data
        for j in sorted(glob.glob(videopath + "/**/*.png", recursive=True)):
            imgs.append(j)
            imgsname.append(os.path.relpath(j, videopath))
    else:
        if not opt.realsense:
            cap = cv2.VideoCapture(0)

    print("Images found: ", len(glob.glob(videopath + "/**/*.png", recursive=True)))
    ...
    while True:
        ...
        output_path = create_output_path(imgs[i_image], opt.outf)
