...
def randomize_background(path, width, height):
    ...
    # Reduce the random rotation range
    angle = 10.0 - random.random() * 20.0  # Rotate between -10 and 10 degrees
    img = crop_to_rotation(img, angle)
    img = scale_to_original_shape(img, width, height)
...
def main(args):
    ...
    out_directory = os.path.join(args.outf, str(args.run_id).zfill(3))
    ...
    i=0
    rot_axis = random.uniform(-2,2)
    pbar = tqdm(total=args.nb_frames, desc=f'Run ID: {args.run_id}', unit='frame')
    ...
    for frame in range(args.nb_frames):
        ...
        # Place object(s)
        #print(f"Generating frame {i+1}/{args.nb_frames} ==== {args.run_id}")
        i+=1
        ...
        if args.backgrounds_folder:
            ...
            if os.path.splitext(background_path)[1].lower() == ".hdr":
                ...
                rotation = [random.random()*0.2-0.1, random.random()*0.2-0.1, rot_axis]
        ...
        im = Image.fromarray(data['colors'][0]).convert('RGBA')  # Convert to RGBA

        if args.backgrounds_folder and os.path.splitext(background_path)[1].lower() != ".hdr":
            # Load the background image and convert it to RGBA
            background = Image.open(background_path).convert('RGBA')
            background = randomize_background(background_path, args.width, args.height).convert('RGBA')  # Ensure background is also RGBA

            # Use alpha_composite for better blending
            im = Image.alpha_composite(background, im)

            im = im.convert('RGB')  # Convert back to RGB if needed for saving or further processing
    ...
    print(f"All frames generated successfully for Run: {args.run_id}.")
    pbar.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--path_single_obj',
        #default="../models/brick_duplo_2x4_jaune/google_16k/textured.obj",
