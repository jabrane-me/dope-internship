...
class ObjectDetector(object):
    ...
    @staticmethod
    def estimate_missing_points(points, cuboid_dimensions=None):
        """Estimate missing points using detected points and geometric properties."""

        # Identify which points are None
        indices_missing = [i for i, p in enumerate(points) if p is None]
        indices_available = [i for i, p in enumerate(points) if p is not None]

        # Cuboid vertex pairs for estimation based on symmetry
        # Assuming indices: 0, 1, 2, 3 are top face corners, 4, 5, 6, 7 are bottom face corners, and 8 is center
        opposite_pairs = [(0, 2), (1, 3), (4, 6), (5, 7), (0, 4), (1, 5), (2, 6), (3, 7)]
        
        # Use the center as a reference point
        center = points[8] if points[8] is not None else np.mean([p for p in points if p is not None], axis=0)

        # Check if we have enough points to perform estimation
        if len(indices_available) >= 5:
            print("Estimating missing points with geometric properties and symmetry.")

            # Estimate missing points based on available pairs and symmetry
            for i in indices_missing:
                # Find pairs of opposite points to use for estimation
                for a, b in opposite_pairs:
                    if a in indices_available and b in indices_available:
                        # Known points a and b
                        pa, pb = np.array(points[a]), np.array(points[b])

                        # Estimate missing point i
                        if i in [0, 2, 4, 6]:  # Vertices usually forming one axis
                            estimated = 2 * pa - pb  # Extrapolate along the line
                        elif i in [1, 3, 5, 7]:  # Opposite vertices
                            estimated = 2 * pb - pa
                        else:
                            # Use symmetry based on the center
                            estimated = center + (pb - center) + (pa - center)

                        # Validate the estimated point
                        estimated = ObjectDetector.validate_estimated_point(estimated, center, cuboid_dimensions)

                        points[i] = tuple(estimated)
                        break
                else:
                    # Fallback to mean of available points if no valid opposite pair is found
                    points[i] = tuple(np.mean([points[j] for j in indices_available], axis=0))

        else:
            print("Not enough points to estimate reliably. Using simple averages.")
            # If not enough points, fill with averages or defaults
            for i in indices_missing:
                points[i] = tuple(np.mean([points[j] for j in indices_available], axis=0))
    ...
    @staticmethod
    def validate_estimated_point(estimated, center, cuboid_dimensions=None):
        """Validate and adjust the estimated point to ensure it fits within expected constraints."""

        # Check if cuboid dimensions are provided
        if cuboid_dimensions:
            # Calculate expected distances from center
            width, height, depth = cuboid_dimensions
            expected_distances = np.array([width, height, depth]) / 2

            # Calculate the actual vector from the center to the estimated point
            vector = estimated - center

            # Normalize the vector to maintain proportions based on expected dimensions
            normalized_vector = vector / np.linalg.norm(vector)

            # Scale the vector to fit within the expected cuboid dimensions
            scaled_vector = normalized_vector * np.min(expected_distances)

            # Update the estimated point
            estimated = center + scaled_vector

        return estimated

    # Example usage in the context of find_object_poses
    @staticmethod
    def find_object_poses(vertex2, aff, pnp_solver, config):
        '''Detect objects given network output'''
        ...
        # Define known dimensions for the cuboid (example dimensions)
        cuboid_dimensions = [3.12, 2.39, 6.37]  # Replace with actual dimensions

        for obj in objects:
            # Run PNP
            points = obj[1] + [(obj[0][0] * 8, obj[0][1] * 8)]

            # Check for missing points and replace them
            if None in points:
                print("Incomplete cuboid detection.")
                print("  result from detection:", points)
                points = ObjectDetector.estimate_missing_points(points, cuboid_dimensions)
