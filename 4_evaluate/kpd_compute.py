...
#     default = "../train2/out_experiment/testset/019", 
#     help='path to prediction data')
# parser.add_argument('--data', 
#     default="../data_generation/blenderproc_data_gen/output/testset/019", 
#     help='path to data ground truth')
# parser.add_argument("--outf",
#     default="results_kpd/",
#     help="base directory to put the data"
#     )
...
# Create a timestamped folder within the base output directory
#timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
timestamp = ""
opt.outf = os.path.join(opt.outf, f"{timestamp}_{len(data_thruth)}")

if not os.path.isdir(opt.outf):
    print(f'creating the folder: {opt.outf}')
    os.makedirs(opt.outf)

tmp_dir = os.path.join(opt.outf, "tmp")
if not os.path.isdir(tmp_dir):
    os.mkdir(tmp_dir)
    print(f'created folder {tmp_dir}/')
...
pickle.dump(count_by_object,open(os.path.join(opt.outf, 'count_all_annotations.p'),'wb'))
pickle.dump(adds_all,open(os.path.join(opt.outf, 'adds_all.p'),'wb'))

count_by_object_guesses["all"] = count_all_guesses
pickle.dump(count_by_object,open(os.path.join(opt.outf, 'count_all_guesses.p'),'wb'))

labels = []
data = []
for key in adds_objects.keys():
    pickle.dump(adds_objects[key],open(os.path.join(opt.outf, f'adds_{key}.p'),'wb'))
    labels.append(key)
    data.append(os.path.join(opt.outf, f'adds_{key}.p'))
