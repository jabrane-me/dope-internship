            ...
            for i_lists in range(len(all_peaks[:-1])):
                ...
                for i_candidate, candidate in enumerate(lists):
                    ...
                    for i_obj in range(len(objects)):
                        ...
                        if point_int[0] >= aff.shape[2] or point_int[1] >= aff.shape[1]:
                            # Handle out-of-bounds case
                            point_int = [
                                min(aff.shape[2] - 1, int(candidate[0])),
                                min(aff.shape[1] - 1, int(candidate[1])),
                            ]
                            print(f"Warning: point_int {point_int} is out of bounds for aff with shape {aff.shape}")
                            continue
                        ...
                        try:
                            v_aff = (
                                np.array([
                                    aff[i_lists * 2, point_int[1], point_int[0]].data.item(),
                                    aff[i_lists * 2 + 1, point_int[1], point_int[0]].data.item(),
                                ]) * 10
                            )
                        except IndexError:
                            print(f"Index out of bounds for aff with shape {aff.shape}, i_lists={i_lists}, point_int={point_int}")
                            continue
